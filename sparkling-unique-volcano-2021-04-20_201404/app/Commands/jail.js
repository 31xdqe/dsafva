...
module.exports.execute = async (client, message, args) => {
    ...
    if (cpuan > 120 ) {
        ...
        if (message.guild.channels.cache.has(ayar.jailLogKanali)) message.guild.channels.cache.get(ayar.jailLogKanali).send(new Discord.MessageEmbed().setAuthor(client.user.tag, client.user.displayAvatarURL({dynamic: true})).setDescription(`${victim}, üyesine <@&830390751006687234> rolü <@796068201750069318> tarafından verildi. \n Sebep: ceza puanını aştı `).setColor("RANDOM"))
    ...
    if (message.guild.channels.cache.has(ayar.jailLogKanali)) message.guild.channels.cache.get(ayar.jailLogKanali).send(embed.setDescription(`${victim}, üyesine <@&830390751006687234> rolü ${message.author} tarafından verildi.(\`#${cezaID}\`) \n Sebep: ${reason} `))
    if (ayar.cezapuanlog && client.channels.cache.has(ayar.cezapuanlog)) client.channels.cache.get(ayar.cezapuanlog).send(`${victim} : aldığınız **#${cezaID}** ID'li ceza ile **${cpuan || '0'}** ceza puanına ulaştınız. `).catch();
